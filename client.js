...
function queryListUsers()
{
    $.post("server_admin.php", { query: "list-users" }, function(data) {
        ...
        if (data.result.length > 0)
        {
            ...
            for(var i = 0; i < data.result.length ; i++)
                {
                markup = '<table style="width:100%; margin-top: 15px; margin-bottom: 20px"><tr><td style="width:33%;">' + data.result[i].id + ' ' + data.result[i].fname + ' ' + data.result[i].lname +'</td><td style="width:33%;">' + data.result[i].email +'</td><td style="width:33%;">' + data.result[i].phone +'</td></tr></table><hr/>';
